...
puts "The second from the last element is #{practice_array.at(-2)}"
puts "The first four elements are '#{practice_array[0..3].join(", ")}'"
puts "If we delete 5, 6, and 7 from the array, we're left with #{practice_array.keep_if {|a| a <= 4 || a >= 8}}"
puts "If we add 5 at the beginning of the array, we're left with #{practice_array.unshift(5)}"
